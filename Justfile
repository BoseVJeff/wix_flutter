...
clean-all: clean
	...
	if(Test-Path windows.zip) {rm -r windows.zip}
	if(Test-Path linux.zip) {rm -r linux.zip}
	if(Test-Path windows.msi) {rm -r windows.msi}

# Only clean temp files generated by this Justfile
clean-temp-files:
...
build-html-web base-href='/':
	flutter build web --release --base-href={{base-href}} --web-renderer=html --csp --no-source-maps --pwa-strategy=offline-first --dart2js-optimization=O2

# Build web debug build
# Uses the profile build flag as release is apparently missing
...
build-debug-web base-href='/':
	flutter build web --profile --base-href={{base-href}} --web-renderer=auto --csp --source-maps --pwa-strategy=none --dart2js-optimization=O0
...
# This recipe assumes a native Linux environment.
# This is intended mainly for GitHub Actions, although it can be used elsewhere
build-linux-bundle-native: build-linux-native zip-linux-bundle-native
...
# This is intended mainly for GitHub Actions, although it can be used elsewhere
build-linux-bundle: build-linux zip-linux-bundle
...
# Generate all builds in a format siliar to when generated by GitHub Actions
build-all-gh:
